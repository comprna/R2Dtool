...
The GTF annotation provided must contain identical gene structures used to generate the transcriptome, including identical transcript names in the FASTA header. One option is to use genomes, transcriptomes and gene structures from the same genome release. Another option is for users to generate their own transcriptome using a genome and gene structure file, e.g. using gffread, or to generate a custom transcriptome using tools like RATTLE, Bambu, or spliced_bam2gff. Note, the liftover requires 'exon' entries in the third column of the GTF, and the exon <-> transcript <-> gene map is identified from column 9 of the GTF.
