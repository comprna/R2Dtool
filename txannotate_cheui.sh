...
bash ${SCRIPTPATH}/cheui_to_bed.sh ${cheui_model_ii_output} ${lift_annotate_output}.tempbed || echo "failed to convert ${cheui_model_ii_output}"

# annotate bed-like transcriptomic sites

Rscript ${SCRIPTPATH}/annotate.R ${lift_annotate_output}.tempbed ${annotation} ${lift_annotate_output}.tempbed.annotate || echo "failed to annotate ${cheui_model_ii_output}"

# liftover annotated transcriptomic sites

Rscript ${SCRIPTPATH}/lift.R ${lift_annotate_output}.tempbed.annotate ${annotation} ${lift_annotate_output} || echo "failed to liftover ${cheui_model_ii_output}"

# finish by deleting all temporary files made by the script
rm ${lift_annotate_output}.*
